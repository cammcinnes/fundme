...
CREATE TABLE PaymentInformation(
  ...
  FOREIGN KEY(PostalCode) REFERENCES PostalCode_Location ON DELETE NO ACTION
);
...
CREATE TABLE Individual(
  ...
  FOREIGN KEY(PaymentInfo) REFERENCES PaymentInformation ON DELETE SET NULL 
);
...
CREATE TABLE Organization_creates_Project (
  ...
  FOREIGN KEY(OUsername) REFERENCES Organization ON DELETE NO ACTION
);
...
CREATE TABLE Organization_creates_Post (
	...
	Timestamp TIMESTAMP,
  FOREIGN KEY(OUsername) REFERENCES Organization,
  FOREIGN KEY(ProjectName) REFERENCES Organization_creates_Project ON DELETE CASCADE
);
...
CREATE TABLE Account_writes_Comment_on_Post (
  ...
  FOREIGN KEY(Username) REFERENCES Account ON DELETE CASCADE,
  FOREIGN KEY(PostID) REFERENCES Organization_creates_Post ON DELETE CASCADE
);
...
CREATE TABLE Individual_makes_Contribution (
  ...
  FOREIGN KEY(IUsername) REFERENCES Individual ON DELETE NO ACTION ,
  FOREIGN KEY(ProjectName) REFERENCES Organization_creates_Project ON DELETE CASCADE
);
...
CREATE TABLE Reward (
  ...
  FOREIGN KEY(OUsername) REFERENCES Organization ON DELETE CASCADE
);
...
CREATE TABLE Individual_receives_Reward (
  ...
  FOREIGN KEY(RewardName) REFERENCES Reward ON DELETE NO ACTION,
  FOREIGN KEY(IUsername) REFERENCES Individual ON DELETE CASCADE
);
...
CREATE TABLE PaymentTier_has_Reward (
  ...
  FOREIGN KEY(PayTierID) REFERENCES PaymentTier ON DELETE CASCADE,
  FOREIGN KEY(RewardName) REFERENCES Reward ON DELETE CASCADE
);
...
VALUES (101, 'wildlife', 'Help Pandas', 'Image of Panda', 'https://i.ebayimg.com/images/g/770AAOSwmYFjG-6w/s-l1600.jpg', CURRENT_TIMESTAMP);

INSERT INTO Organization_creates_Post(PostID, OUsername, ProjectName, Content, ImageURL, Timestamp)
VALUES (102, 'globewarm', 'Stop Global Warming', 'Time is running out.', 'https://atmos.earth/wp-content/uploads/2022/10/mm93_evil_corporate_greed_stealing_a_habitable_future_5997b7c6-e742-4ce4-b948-8d22a4abaef4.jpg', CURRENT_TIMESTAMP);

INSERT INTO Organization_creates_Post(PostID, OUsername, ProjectName, Content, ImageURL, Timestamp)
VALUES (103, 'globewarm', 'Stop Global Warming', 'For a green future.', 'https://imageio.forbes.com/specials-images/imageserve/61a7d717a109f9017391549b/comparing-green-earth-and-effect-of-air-pollution-from-human-action--glbal-warming/960x0.jpg?height=481&width=711&fit=bounds', CURRENT_TIMESTAMP);

INSERT INTO Organization_creates_Post(PostID, OUsername, ProjectName, Content, ImageURL, Timestamp)
VALUES (104, 'ubcneedsmoney', 'Open more CPSC110 sections', 'Overcrowded CPSC Lecture Hall.', 'https://i.cbc.ca/1.4826104.1537136387!/fileImage/httpImage/image.png_gen/derivatives/16x9_940/lumohacks-hackathon-sfu.png', CURRENT_TIMESTAMP);

INSERT INTO Organization_creates_Post(PostID, OUsername, ProjectName, Content, ImageURL, Timestamp)
VALUES (105, 'fundbenslife', 'Bens Fund', 'Help me get to class.', 'https://m.media-amazon.com/images/I/71iEjFb5AAL.jpg', CURRENT_TIMESTAMP);

-- Account_writes_Comment_on_Post <--------------

INSERT INTO Account_writes_Comment_on_Post(CommentID, Username, PostID, Timestamp,Content)
VALUES (0, 'tshebs', 105, CURRENT_TIMESTAMP, 'Were gonna get ye that red bike and we hope to c ye riding it around brum.');
